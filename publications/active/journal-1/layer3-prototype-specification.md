...
**Status:** Batch 1 CLOSED — fidelity trace and failure-semantics contract repaired (2026-09-11). Batch 2 REMAINS OPEN — OPEN-L3-3 CAUTION-Go authority semantics unresolved (2026-09-11)  
**Closed on:** Batch 1: 2026-09-11 | Batch 2: OPEN  
*(Historical — superseded: Batch 2 CLOSED WITH BOUNDED OPEN ITEMS — advisory rule evidence and semantics partially specified. Status changed on residue-repair discovery that OPEN-L3-3 blocks Batch 3.)*  
...
| OPEN-L3-3 | **NEW — OPEN** | **CAUTION-Go presentation vs. rule semantics.** Appendix C §C.4 (line 791) states: "When S = CAUTION, the Go recommendation is automatically presented by the system with a caution qualifier." Authority classification: **B (UI/presentation guidance)** — the statement describes how Go should be rendered *if* produced by Layer 3 rules, not that Go is mandated in every CAUTION episode. However, the phrase "automatically presented" is ambiguous with a normative reading (Go is always shown in CAUTION state regardless of rule engine output). This ambiguity must be resolved before CAUTION-state UI is implemented. **Classification B is the current working interpretation; the normative reading (A) cannot be ruled out without further clarification.** Appendix C must not be modified to resolve this open item without a separate design record. **Blocks CAUTION Go UI implementation.** Does not block R-CAUTION-001 through R-CAUTION-004 rule specification (which produce Delay). |

OPEN-L3-1 is now PARTIALLY RESOLVED. OPEN-L3-1C and OPEN-L3-1D carry the unresolved portions forward. OPEN-L3-3 blocks CAUTION advisory behaviour in Batch 3. Batch 3 engine implementation must not begin until OPEN-L3-3 is resolved — the blocking nature of OPEN-L3-3 cannot be partially routed around by implementing SAFE-only or Delay-only paths without explicit authorisation from a later task.
...
| Semantic verification | `semantic-verification-batch2.json` | 89 PASS / 0 FAIL / 1 OPEN (OPEN-L3-3 tracking check) — 90 checks total after Repair 5 |
