...
PENDING WORK IN THIS FILE
  ...
  [x] NEW SECTION: Empirical Characterisation, after Domain Instantiation.
      ...
      binds 7.72% of departure hours (5 yr) / 5.98% (3.25 yr, finer waves).
  ...
  [x] C3 Flehmig-style baseline BUILT AND RUN — scripts/condition_comparison.py.
      ...
      (24.64%) from Level 2 (7.72%). P21-P24 registered before the run and
...
**Result 1 — the CAUTION mode is reached, and Level 2 governance binds.** For a small vessel in the 05:00–09:00 departure window, *S* = *CAUTION* in **7.72%** of hours under the primary configuration and **5.98%** under the resolution configuration. In those hours the architecture withholds *DepartureTime* and *Duration* while continuing to supply *Go* and *Delay*. A binary architecture, having no state between participation and withdrawal, supplies the full recommendation set in every one of them. This is the governance difference the architecture exists to produce, expressed as a rate rather than as a design claim.

**Result 2 — the participation gate is reachable by sea state, not only by darkness.** *UNSAFE* occurs in 1,170 daylight hours (5.82%) under the primary configuration and 409 hours (3.13%) under the resolution configuration. Weather-driven classifications account for 11.8% and 7.0% of all *UNSAFE* hours respectively; the remainder is the time-of-day component *g*_t. Both governance states are therefore attainable at the deployment site under conditions the operator would actually encounter, rather than only after dark.

**Result 3 — vessel conditioning is operative.** Small and big vessels receive different classifications in **12.16%** of departure-window hours under the primary configuration (7.92% under the resolution configuration). Under vessel-blind thresholds this figure is zero by construction, so the vessel-conditional form of *g*_o(*o*, *v*) changes governance output rather than only notation.
...
|---|---|---|---|
| *g*_o (ocean state) | **98.66% / 97.41%** | 17.84% / 12.59% | Binds |
| *g*_t (time of day) | 0% / 0% | **87.63% / 91.10%** | Binds |
...
**Result 5 — resolution sensitivity.** The wave height input carries 98.66% of daylight *CAUTION* decisions under the primary configuration (97.41% under the resolution configuration), which makes its spatial resolution material. The two wave models correlate at *r* = 0.953 across 28,501 valid hourly pairs, but differ in the upper tail: the 50 km model produces a five-year maximum of 2.60 m against 1.84 m for the 8 km model, consistent with a coarse cell averaging open water that the finer model resolves as sheltered by the offshore island group. Holding thresholds fixed, the Level 2 binding rate moves from 7.72% at 50 km to 5.98% at 8 km, and daylight *UNSAFE* from 1,170 hours to 409. Every figure in this section is therefore given for both configurations. The direction is consistent: the coarser model classifies more conservatively, because averaging open water inflates the wave heights experienced by vessels operating inside the sheltered zone. Reporting both quantifies the dependence rather than leaving it as an unexamined assumption, and the 1.7-point spread bounds it.
...
**TABLE VII.** Admissible-set divergence between conditions, departure window 05:00–09:00, small vessel, primary configuration (9,133 hours). The resolution configuration gives the same structure with Level 2 at 5.98%.

| | C0 ungated | C1 binary | C3 traffic-light | C2 proposed |
|---|---|---|---|---|
| **C0 ungated** | — | 24.64% | 24.64% | 32.36% |
| **C1 binary** | 24.64% | — | **0.00%** | 7.72% |
| **C3 traffic-light** | 24.64% | 0.00% | — | 7.72% |
| **C2 proposed** | 32.36% | 7.72% | 7.72% | — |
...
The comparison also isolates the two governance levels. C0 against C1 differs on 24.64% of departure hours, which is the contribution of the participation gate alone. C0 against C2 differs on 32.36%. The **7.72% difference between them is attributable to Level 2 alone** (5.98% under the resolution configuration) — advisory scope restriction, operating in hours where every other condition, including the traffic-light baseline, leaves the full recommendation set available.
...
**A discontinuous governance boundary in the approved time classifier.** A fifth threat concerns an approved but not-yet-applied change to the time component, and is stated here because it bears directly on the graduated-governance claim this paper makes. The time thresholds reported above (06:00 / 17:00 / 19:00) have no located source; a specification decision has therefore been approved replacing them with a solar-event classifier that is SAFE from sunrise to sunset and UNSAFE otherwise. **That classifier transitions directly between SAFE and UNSAFE, with no intermediate value.** In counterfactual analysis over the five-year record, direct SAFE→UNSAFE transitions rise from **2 to 1,536**, while time-driven SAFE→CAUTION transitions fall from **1,545 to 0**. The architecture's central argument is that graduated governance is preferable to a binary step, and under this design the most predictable transition of the day becomes a step — so the tension is real and is disclosed rather than absorbed. Three qualifications bound it. First, the increase follows from **removing an intermediate time band for which no evidence was found** — three reviews located no source supporting a twilight CAUTION state — and not from any claim that conditions change discontinuously at sunset; nothing physical, meteorological or physiological is asserted to be discontinuous there. Second, the discontinuity is confined to the time component: **the architecture remains globally three-state**, with CAUTION reached through the wave and rainfall classifiers in 2,091 of 43,848 hours (4.77%) under the same analysis, all weather-driven, and the containment property A_AI(SAFE) ⊃ A_AI(CAUTION) ⊃ A_AI(UNSAFE) = ∅ is untouched. Third, the residual concern is a usability one — predictable withdrawal of advisory support at sunset may warrant anticipatory interface support, which is a notification concern separate from the safety-state classifier and is deliberately not addressed by inventing an intermediate state to smooth the transition. **The figures in this paragraph are counterfactual sensitivity results for the approved design; the classifier characterised elsewhere in this paper is the incumbent fixed-clock one, and the reported binding rates (7.72% / 5.98%) are computed under it.**
...
Third, the architecture is instantiated for small-scale coastal fisheries at Kota Kinabalu and its governance layer executed against five years of hourly environmental data for that site. The *CAUTION* mode binds in 7.7% of departure-window hours for the operative vessel class — approximately one departure morning in thirteen on which a binary architecture supplies tactical recommendations that this architecture withholds — falling to 6.0% when a higher-resolution wave model is substituted over a shorter period, a spread that is reported as the grid-resolution sensitivity rather than resolved to a single figure. The *UNSAFE* state is reachable by sea state and not only by darkness, and the vessel-conditional form of the ocean-state component changes the classification for 12.2% of departure hours relative to vessel-blind thresholds. A four-condition comparison over the same record isolates the two governance levels: the participation gate accounts for 24.64% of departure hours and advisory scope restriction for a further 7.72%, while a traffic-light baseline instantiating the closest structural precedent diverges from a plain binary gate in **0.00%** of hours — its intermediate level is not observable in AI output. Predictions were registered with numeric bounds before each analysis ran; twenty-two of twenty-four were confirmed and two refuted; separately, two predictions were re-resolved when provenance audits corrected the thresholds they had been scored against, one in each direction. These figures characterise governance behaviour at one location and, with the marine-warning component unmeasured throughout, are lower bounds; no incident record exists against which the classifications themselves could be scored.
