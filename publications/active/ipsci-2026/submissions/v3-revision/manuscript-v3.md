...
PENDING WORK IN THIS FILE
  ...
  [x] NEW SECTION: Empirical Characterisation, after Domain Instantiation.
      ...
      binds 7.84% of departure hours (5 yr) / 6.15% (3.25 yr, finer waves).
  ...
  [x] C3 Flehmig-style baseline BUILT AND RUN — scripts/condition_comparison.py.
      ...
      (24.64%) from Level 2 (7.84%). P21-P24 registered before the run and
...
**Result 1 — the CAUTION mode is reached, and Level 2 governance binds.** For a small vessel in the 05:00–09:00 departure window, *S* = *CAUTION* in **7.84%** of hours under the primary configuration and **6.15%** under the resolution configuration. In those hours the architecture withholds *DepartureTime* and *Duration* while continuing to supply *Go* and *Delay*. A binary architecture, having no state between participation and withdrawal, supplies the full recommendation set in every one of them. This is the governance difference the architecture exists to produce, expressed as a rate rather than as a design claim.
...
|---|---|---|---|
| *g*_o (ocean state) | **97.40% / 95.05%** | 17.82% / 12.57% | Binds |
| *g*_t (time of day) | 0% / 0% | **87.55% / 90.96%** | Binds |
| *g*_r (rainfall) | 3.21% / 6.21% | 0.34% / 0.48% | Binds |
| *g*_w (wind) | 0% / 0% | 0% / 0% | Measured; threshold never reached |
| *g*_m (marine warning) | 0% / 0% | 0% / 0% | **Not measured** — no archive |

*g*_w never determines a classification because sustained wind at this site does not reach the MET Malaysia Category 1 criterion: the five-year maximum is **21.8 kn against a 22 kn threshold**. This was tested rather than assumed. An earlier collection sampled wind from a land grid cell approximately 13 km from the marine cell; correcting it to a sea cell raised mean wind by 57.3%, and a prediction was registered beforehand that the corrected series would cross the threshold. It did not, and that prediction was refuted. The margin is 0.2 kn, and the honest statement is that sustained wind essentially never reaches the threshold at this site — not that wind is irrelevant to the domain. The component is retained: MET's wind criteria exist because those conditions occur in Malaysian waters, and an instantiation of this architecture on an exposed coast would exercise *g*_w routinely.
...
**Result 5 — resolution sensitivity.** The wave height input carries 97.40% of daylight *CAUTION* decisions under the primary configuration (95.05% under the resolution configuration), which makes its spatial resolution material. The two wave models correlate at *r* = 0.953 across 28,501 valid hourly pairs, but differ in the upper tail: the 50 km model produces a five-year maximum of 2.60 m against 1.84 m for the 8 km model, consistent with a coarse cell averaging open water that the finer model resolves as sheltered by the offshore island group. Holding thresholds fixed, the Level 2 binding rate moves from 7.84% at 50 km to 6.15% at 8 km, and daylight *UNSAFE* from 1,170 hours to 409. Every figure in this section is therefore given for both configurations. The direction is consistent: the coarser model classifies more conservatively, because averaging open water inflates the wave heights experienced by vessels operating inside the sheltered zone. Reporting both quantifies the dependence rather than leaving it as an unexamined assumption, and the 1.7-point spread bounds it.
...
**TABLE VII.** Admissible-set divergence between conditions, departure window 05:00–09:00, small vessel, primary configuration (9,133 hours). The resolution configuration gives the same structure with Level 2 at 6.15%.

| | C0 ungated | C1 binary | C3 traffic-light | C2 proposed |
|---|---|---|---|---|
| **C0 ungated** | — | 24.64% | 24.64% | 32.48% |
| **C1 binary** | 24.64% | — | **0.00%** | 7.84% |
| **C3 traffic-light** | 24.64% | 0.00% | — | 7.84% |
| **C2 proposed** | 32.48% | 7.84% | 7.84% | — |
...
The comparison also isolates the two governance levels. C0 against C1 differs on 24.64% of departure hours, which is the contribution of the participation gate alone. C0 against C2 differs on 32.48%. The **7.84% difference between them is attributable to Level 2 alone** (6.15% under the resolution configuration) — advisory scope restriction, operating in hours where every other condition, including the traffic-light baseline, leaves the full recommendation set available.
