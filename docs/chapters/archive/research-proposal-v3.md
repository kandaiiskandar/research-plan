...
**O2.** To formally define the environmental state vector E = {w, r, m, o, v, t}, safety state function S = f(E), participation gate function G(S), and AI-admissible recommendation space A_AI(S), with the containment property A_AI(SAFE) ⊃ A_AI(CAUTION) ⊃ A_AI(UNSAFE) = ∅, and to establish the Safety Dominance Property AI(E) ⊆ A_AI(S) by construction for the symbolic advisory reasoning engine through Rule-Set Starvation: the governance layer supplies only the state-specific rule set RS(S) to the engine before inference begins, so no rule in RS(CAUTION) can produce outputs beyond {Go, Delay}. *(RQ2: Formal specification)*
...
Layer 3 is the AI Advisory Reasoning Engine (Inference Layer). This research implements Layer 3 as a symbolic rule-based engine, enabling proof by construction of the Safety Dominance Property via Rule-Set Starvation: the Governance Layer supplies only the restricted rule set RS(CAUTION) before inference begins, withholding the production rules in RS(SAFE) \ RS(CAUTION) required to compute DepartureTime and Duration. No rule in that configuration can generate outputs beyond {Go, Delay}, so AI(E) ⊆ A_AI(S) holds by construction. Because the Governance Layer is separated from the reasoning engine and operates before inference, the same governance pair (G(S), A_AI(S)) is in principle applicable to probabilistic reasoning engines through alternative pre-inference enforcement mechanisms; that generalisation is treated as future work and is outlined in Appendix B, but is not part of the formal specification or evaluation deliverables of this thesis.
...
## Appendix B: Architectural Interpretation and Generalisability Note
...
| State S = CAUTION | Warning indicator | Rule-Set Starvation toggle in the symbolic AI Advisory Reasoning Engine |
| Governance Layer (Layer 2) | Filter | Deterministic Safety-State Gating Layer: non-AI, assurably guarded |
| Inference Layer (Layer 3) | Recommendation module | AI Advisory Reasoning Engine: symbolic rule-based (this thesis) |
| Containment property | Feature list | Formal enclosure guaranteeing Safety Dominance via Rule-Set Starvation |

**Generalisability note (out of scope for this thesis).** Because the Governance Layer is separated from the advisory reasoning engine and operates before inference, the architectural pattern is not tied to a symbolic Layer 3. In principle, a probabilistic ML advisory model could be governed by the same pair (G(S), A_AI(S)) through an alternative pre-inference mechanism such as constrained output decoding (logit masking), in which the generation probabilities of recommendation categories outside A_AI(S) are set to zero before decoding resolves. This thesis does not formally specify, prove, implement, or evaluate that mechanism; it is noted here only to characterise the architectural pattern's generalisability and to indicate a direction for future work. The formal specification (O2), prototype (O3), and comparative evaluation (O4) all concern the symbolic rule-based implementation.
