...
Small-scale coastal fishers in Malaysia make safety-critical departure decisions three to six times per week without institutional support, yet existing AI governance architectures offer only binary control: the AI either generates its full recommendation set or shuts down entirely, leaving intermediate-risk conditions unaddressed. The proposed architecture addresses this through two coupled controls: a participation gate G(S), which governs whether the AI operates at all, and an admissible recommendation space A_AI(S), which governs what the AI may say if it does. Both are determined by a classified environmental safety state computed from six observable inputs: wind speed, rainfall intensity, marine warning level, ocean state, vessel category, and time of day. Three governance modes follow: SAFE (full advisory scope), CAUTION (go/no-go and delay only), and UNSAFE (AI silent). As risk increases, the set of recommendations the AI may produce narrows toward nothing, a guarantee the architecture enforces by construction and formally terms the Safety Dominance Property. The architecture is reasoning-engine-agnostic, supporting both symbolic rule-based and probabilistic machine learning implementations. A Design Science Research methodology guides the work from architecture design and formal specification through prototype implementation, three-condition comparative evaluation (ungated, binary-gated, and two-level graduated), and contextual user validation with small-scale fishers in coastal Malaysia. The functional prototype is validated using a historical 2020–2024 environmental dataset as a deterministic replay testbed to verify the graduated safety boundaries across real-world weather transitions. The expected outcome is a formally verifiable, offline-capable architecture in which AI advisory scope narrows as environmental risk increases, producing an intermediate governance mode with no counterpart in the reviewed literature.
...
**Scholarly support.** The gap is confirmed across multiple independent bodies of literature. Indykov et al. (2025), reviewing 206 papers and 16 architectural tactics for ML-enabled systems, found no tactic demonstrating formal impact on safety through advisory scope restriction. Shamsujjoha et al. (2025) identify 13 distinct runtime guardrail actions across 32 studies; none conditions advisory scope on classified environmental safety state. Perez-Cerrolaza et al. (2024) survey AI safety mechanisms across automotive, avionics, railway, and industrial domains in 294 references: safety bags, safety monitors, and safety envelopes: all binary. Flehmig et al. (2024), the closest structural precedent, propose a three-level traffic-light degradation index for AI in safety-critical industrial systems. Their intermediate level (orange) heightens supervisory activity but leaves the AI's advisory scope unchanged between Level 1 and Level 2. Across four fisheries application domains, Haque and Al Jufaili (2026) confirm the same absence. Rahim et al. (2024) document the institutional equivalent for coastal fishers: the only external advisory is a binary government warning to stop fishing, with no graduated response.
...
Layer 3 is the AI Advisory Reasoning Engine (Inference Layer). The architecture is implementation-agnostic at this layer, supporting both rule-based and probabilistic ML implementations. The Deterministic Safety-State Gating Layer enforces the Safety Dominance Property regardless of which is used: via Rule-Set Starvation for symbolic implementations, and via constrained output decoding (logit masking on output recommendation categories) for probabilistic ML implementations.

When Layer 3 is implemented as a probabilistic ML model (for example, a probabilistic recommendation model trained on historical departure data), the Governance Layer enforces A_AI(S) through constrained output decoding (logit masking), algebraically setting the generation probabilities of unsafe output recommendation categories to zero before inference resolves. This ensures the Safety Dominance Property holds for ML implementations by restricting the model's admissible output space at the decoding stage rather than the rule definition stage.

The prototype implementation uses a rule-based engine, enabling proof by construction of the Safety Dominance Property via Rule-Set Starvation: the Governance Layer supplies only the restricted rule set RS(CAUTION) before inference begins, withholding the production rules in RS(SAFE) \ RS(CAUTION) required to compute DepartureTime and Duration. RS(CAUTION) contains only rules producing recommendations within {Go, Delay}; no rule in that configuration can generate DepartureTime or Duration, so the Safety Dominance Property AI(E) ⊆ A_AI(S) holds by construction.
...
The governance layer performs only threshold comparisons and lookups (O(1) complexity), requires no connectivity, runs on smartphone-class hardware, and operates from parameters fishers already observe. Table 2 maps each formal component to its architectural interpretation to clarify the contribution boundary.
...
*Table 2: Architectural interpretation of formal components* Pre-hoc scope restriction under CAUTION also eliminates compute spent generating recommendation types that governance would suppress, a direct efficiency advantage in battery-constrained at-sea conditions (Katende, 2026).

---

## 8. Research Design and Methodology

The research follows a DSR methodology (Peffers et al., 2007) structured across five phases corresponding to the five research questions.

Phase 1 (RQ1) covers architecture design through layered system modelling and conceptual framework development. The three-mode governance structure (SAFE/CAUTION/UNSAFE) is designed using the four-layer pipeline model, grounded in the literature gap analysis. This phase is complete.

Phase 2 (RQ2) covers formal specification: mathematical formalisation of E, S = f(E), G(S), and A_AI(S). Safety thresholds are operationalised using Malaysian maritime safety standards (MET Malaysia Category 1 warning thresholds for small craft) and expert elicitation. The Safety Dominance Property is proved by construction for the symbolic prototype, with conditions specified for the probabilistic ML extension. This phase is complete.

The transition between safety states is governed exclusively by S = f(E), which maps E = {w, r, m, o, v, t} into S ∈ {SAFE, CAUTION, UNSAFE}. This decouples domain safety logic from advisory inference entirely. The function f(E) acts as an immutable pre-inference circuit breaker. Under CAUTION, the system pairs a positive participation flag (G(S) = 1) with a structurally restricted admissible advisory scope (A_AI(CAUTION) = {Go, Delay}).

This restriction is enforced through Rule-Set Starvation at the reasoning layer: the system supplies only RS(CAUTION) to the advisory engine before evaluation begins, withholding the production rules in RS(SAFE) \ RS(CAUTION) required to compute DepartureTime and Duration. By starving the engine's reasoning environment of the rules needed to produce high-specificity outputs, the architecture enforces the formal containment property A_AI(SAFE) ⊃ A_AI(CAUTION) ⊃ A_AI(UNSAFE) = ∅, ensuring the Safety Dominance Property AI(E) ⊆ A_AI(S) is satisfied and the system cannot produce over-precise outputs when the environmental state is classified as CAUTION.
