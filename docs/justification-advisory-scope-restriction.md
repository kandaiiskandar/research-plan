...
Tatasciore & Loft (2025) [[notes]](../notes/Calibrating%20Reliance%20on%20Automated%20Advice-%20Transparency%20and%20Trust%20Calibration%20Feedback.md) provide the most direct experimental evidence that informational interventions cannot overcome automation bias. In a controlled experiment (N=160, mixed ANOVA), they tested whether explicitly telling participants "your trust is inappropriate" and instructing them to "take your time and check the information more carefully" would improve automation use accuracy. The result: zero measurable effect on any outcome variable — hit rate, correct rejection rate, sensitivity (d′), decision time, workload, trust, or usability. Even more critically, their Signal Detection Theory analysis reveals that increased transparency (providing the AI's rationale and calculations) actually *increased* automation agree-bias (criterion c shifted from −0.15 to −0.33, η²=0.22, large effect): participants shown richer AI justifications became *more* likely to uncritically accept incorrect advice. This finding directly undermines the alternative approach of "reduce autonomy and provide more transparency" — richer information provision exacerbates, rather than mitigates, the very automation bias that scope restriction is designed to prevent.
...
| CLGuard / Seong, Lim & Yoon (2025) [[notes]](../notes/clguard-extraction.md) | Per-action VLM semantic suppress/permit via CLG score + kinematic thresholds | Binary suppress or pass; no S = f(E); no environmental state classification; self-referential (AI's own CLG scores feed context buffer); no intermediate scope restriction |
| CRANE / Banerjee et al. (2025) [[notes]](../notes/CRANE-%20Reasoning%20with%20Constrained%20LLM%20Generation.md) | Output grammar G restricts admissible outputs; augmented grammar G_a = R_M·G allows reasoning within constrained scope; formal proof (Prop. 3.1) binary constraint reduces capability to TC⁰ | Syntactically-triggered 2-mode alternation (delimiter-based), not environmentally-state-conditioned; no CAUTION mode; strongest theoretical motivation for restricted-but-non-empty scope |
