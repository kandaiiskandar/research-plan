...
Maqsood et al. (2026) [[notes]](../../notes/A%20Hybrid%20AI%20and%20Rule-Based%20Decision%20Support%20System%20for%20Disease%20Diagnosis%20and%20Management%20Using%20Labs.md) provide a parallel from clinical decision support. Their system separates a clinically validated rule base (which confirms diagnoses directly when lab conditions match explicit if-then rules — deterministic, auditable, verifiable by clinicians) from an XGBoost ML classifier (which generates likely-diagnosis suggestions when certainty is lower). The rule base cannot produce outputs beyond the ICD-10 codes its rules are written to confirm; the ML component produces a ranked probability list that acts as advisory input to the physician. This functional separation — deterministic rule layer for confirmed facts, AI layer for advisory suggestions — is the same structural principle as the Layer 2 / Layer 3 division in this architecture, applied in a safety-sensitive clinical domain. Maqsood et al. motivate the hybrid on interpretability grounds: rule-based components carry higher physician acceptance because the rules can be clinically validated, while pure ML systems "often lack interpretability and trust." The same argument applies here: the Layer 2 governance function can be verified by exhaustive threshold testing, while Layer 3 advisory reasoning operates within a rule space whose boundary was set before any inference began.
