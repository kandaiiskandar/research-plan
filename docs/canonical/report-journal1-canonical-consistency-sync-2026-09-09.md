...
## Independent Semantic Closure Repair

*Appended 2026-09-10. §12 and §17 above are left exactly as written, including the claim this section corrects.*

### 1. Independent-review trigger

Two active semantic residues in the maintained manuscript: an **epistemic overreach in the Safety Dominance composite guarantee**, and a **domain-transfer writing instruction** in the Section 13 plan.

### 2. False negative in the previous audit — and its actual cause

§12 above states that *"the phrase 'warranted scope' does not occur"*. **That was false**, and the cause is worth recording precisely because it is not the one that would be guessed.

**The regex matched.** The audit ran `grep -nEi "warranted|justified scope|epistemic|configured admissible|admissible scope"` and the Composite Guarantee line **was** among its three hits. But the command piped results through `sed 's/\(.\{170\}\).*/\1…/'`, and the word *warranted* sits beyond character 170 of a 400-character line. The line was displayed as `"…uses totality to establish that a state exists, together with the state-indexed rule-set and ga…"` — and classified as correct on the strength of that fragment.

**The search was right; the evidence display was truncated, and the classification was made on the truncation.** Every earlier detector failure on this workstream was a matching failure — Markdown-versus-Python headers, markers below their content, docstrings. This one is different in kind: **a search that truncates its own output can report a hit and hide the reason it hit.** The sweep for this repair therefore matches and prints on full lines, with no truncation anywhere.

### 3–4. The two residues

**J1-S1.** *"…no formally identifiable path by which an AI recommendation can exceed the scope **warranted by the current environmental conditions**."* This claims the environment determines which advice is *correct*. Theorem 6.3 proves `AI(E) ⊆ A_AI(f(E))` under the RS(S) and Layer 3 assumptions — enforcement of a configured mapping, not the soundness of that configuration.

**J1-S2.** *"Generalisation: which aspects of the architecture are **domain-independent**."* A writing instruction, and therefore the more dangerous form: it would have propagated the retired overclaim into Discussion prose not yet written.

### 5. Repairs

1. **J1-S1** → *"the **configured admissible scope associated with the current governance state**."*
2. **A second overclaim in the same sentence, not in the brief.** *"Monotonicity ensures that state **appropriately** restricts scope as risk increases."* Theorem 6.2 proves `A_AI(S₁) ⊆ A_AI(S₂)` — the *direction* of change, not that the restriction is fitting. "Appropriately" is the same epistemic claim in a quieter word. Replaced with *"the configured admissible sets contract, never expand, as the classified state worsens."*
3. **A new bounding paragraph** stating what the three theorems do *not* establish: that `A_AI(CAUTION) = {Go, Delay}` is epistemically warranted, optimal or evidence-derived; it is a conservative architecture policy whose derivation remains outstanding. Removing a word leaves a gap the same claim can drift back into; stating the boundary positively closes it.
4. **J1-S2** → re-instantiable structures separated from domain-specific thresholds, inputs, evidence and findings, with an explicit note that this study establishes no empirical portability beyond its site.

**The theorems, their statements and their proofs were not touched** — only the prose interpreting them.

### 6. Assertion-equivalent searches

Full-line, untruncated, across the entire Journal 1 tree. **Epistemic class: 0 active. Domain-transfer class: 0 active.** Zero hits in the historical and planning files as well. Eleven findings classified in `semantic-closure-audit.csv`.

### 7–8. Deliberately preserved

**Condition mapping unchanged** — labels not renamed (J1-SR3). **The Flehmig baseline remains deferred**, its open item intact.

Worth stating plainly: **canonical consistency can close while that evaluation-design extension stays open.** The missing baseline is not canonical drift — Journal 1's experiments have not been executed, so there is no result to be inconsistent with. It is future research-design work, and this closure does not claim to have resolved it.

### 9. Protected state

Appendix C **`abee8715e842e0d9`** and conference manuscript **`27b33b846ae327cb`** unchanged, verified before and after. Prediction register, solar artefact, canonical scripts and raw datasets byte-identical. Canonical state intact: `g_r(r, κ)`, `χ(c) = 1 iff c ∈ {95,96,99}`, `D = {m}`, 21.6 / 27.0 kn, wave UNSAFE > 1.25 m, Model B `g_t` with exact sunrise SAFE and exact sunset UNSAFE, Level 2 **5.81% / 4.48%**, register **24 · 15 / 9**. Changed: the two files already on this branch. Unexpected: none. Four CSVs parser-tested: PASS.

### 10. Final closure decision

All criteria pass. No stop condition triggered.

**The distinction this repair turns on** is the one in the task's own guiding principle, and it is worth keeping visible in the paper rather than only in the audit trail: a theorem can prove that a configured rule is enforced; it cannot, without separate evidence, prove that the configured rule is the right one. The manuscript now says both halves of that.
