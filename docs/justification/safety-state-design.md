...
**Reference thresholds** (UNSAFE/CAUTION boundary anchored to MET Malaysia official warning criteria; SAFE–CAUTION boundary requires domain expert validation):
...
**Basis for the CAUTION boundary.** The SAFE–CAUTION boundary sits below the official warning thresholds — conditions that are elevated but have not yet triggered a formal MET warning. The wave height boundary of 1.5 m and wind boundary of 22 knots represent the pre-warning zone where detailed recommendations (departure time, duration, fishing area) begin to lose reliability while directional guidance (go/no-go) remains valid. These sub-threshold values require validation against local domain expert knowledge of Kota Kinabalu coastal conditions and are consistent with the behavioural transition documented by Gao (2024) [[notes]](../../notes/Mapping%20the%20decision-making%20factors%20of%20small-scale%20fishers-%20a%20case%20study%20of%20Penang.md) where fishers adopt cautious-go behaviour below formal warning thresholds.

**Vessel category note.** The v variable classifies the vessel, not a transient condition. For small vessels (≤ 22 ft / below 40 GRT — the dominant class in Sabah's small-scale fishery per Shaffril et al. 2017 and Yamin et al. 2025 [[notes]](../../notes/Interplay%20of%20traditional%20knowledge%20and%20adaptive%20capacity%20in%20climate%20change%20adaptation%20of%20small-scale%20fishers%20in%20central%20Terengganu%2C%20Malaysia%20.md)), the vessel parameter alone elevates the overall state to CAUTION via worst-case aggregation — reflecting MET's own Category 1 criterion that specifically targets small craft as the most vulnerable vessel class. There is no UNSAFE classification for vessel category alone; UNSAFE is triggered by environmental parameters, not vessel type.

These specific values are parameters of the architecture, not structural features. The architecture's formal properties (containment, Safety Dominance) hold for *any* valid threshold assignment. The UNSAFE boundary values are grounded in MET Malaysia's authoritative published criteria; the CAUTION boundary values require domain expert calibration and empirical validation.
